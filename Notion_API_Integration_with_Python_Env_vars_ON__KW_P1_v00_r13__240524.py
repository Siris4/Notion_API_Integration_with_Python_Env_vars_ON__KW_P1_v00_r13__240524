...
def backup_notion_data():
    # Backup current Notion page and database content
    ...
    print("Notion page content backed up successfully.")
...
def scrape_favorites():
    # Replace with the URL of the Notion page with your favorites
    driver.get(
    ...
    favorites = []
    try:
        favorite_elements = driver.find_elements(By.XPATH, "//div[@class='sidebar']//div[contains(@class, 'favorite')]")
        for element in favorite_elements:
            element.click()
            time.sleep(1.5)
            link_element = driver.find_element(By.XPATH, "//button[@class='more-button']").click()
